...
import {setListaAcreditacionEnviado} from "../../action/ActionAcreditacion";
...
class  EnviarMensaje  extends Component{
      ...
      // e.preventDefault();
        this.props.form.validateFields((err, values) => {
          if (!err) {
                     ...
                     store.dispatch(setListaAcreditacionEnviado(registro));
                    this.setState({ loading: true });
                    setTimeout(() => {
                      this.setState({ loading: false, visible: false });
                    }, 3000);
      ...
      render(){
        ...
        const header={ids:this.props.itemSeleccionado.map(item=>item.idAcreditacionPago).join(",")}
           return (
            ...
            <Modal width={450}
              visible={visible}
              title="Enviar Mensaje Acreditacion de Pago de Deuda"
 ...
 export  default Form.create({ name: 'form_programacion' })(EnviarMensaje);